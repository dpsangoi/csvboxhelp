...
{% tab title="Angular" %}
The **`isImported`** property provides access to the **`result`** and **`data`**variables via the specified callback function.
...
Triggers when the data is pushed to the destination. Two objects are available in this event:
...
<strong>  onClose={()=>{
</strong>    console.log("Modal closed");
  }}
  onSubmit={(data)=>{
    console.log("onSubmit");
    console.log(data.import_id);
...
@Component({
  ...
  template: `
    <csvbox-button
      ...
      [dynamicColumns]="dynamicColumns"
      [isImported]="isImported.bind(this)"
      [isClosed]="isClosed.bind(this)"
      [isSubmitted]="isSubmitted.bind(this)"
      [isReady]="isReady.bind(this)">
      Import
    </csvbox-button>
  `
})
...
function App() {
  return (
    <div className="App">
      <CSVBoxButton
        licenseKey="8qCmJIkmaknwvbkRtD2RuWC6WHfRnT"
        user={{
          user_id: "default123"
        }}
        onImport={(result, data) => {
